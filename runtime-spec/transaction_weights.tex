...
Depending on the Runtime function some parts of the code will excessively increase execution time. Potential indicators that impact the execution time are for example loops, I/O operations and data manipulation. Transaction weights should take into consideration operations that could have heavy implications on those design choices (Fatemeh? This is confusing.). The final assigned Weights are calculated by benchmarking the Runtime functions, which is described in Section \ref{sect:benchmarking}.
\newline

Section \ref{sect:examples-runtime-analysis} walks through two practical examples of Runtime function
analysis. Not every possible outcome (Fatemeh: outcome for what? Be more precise.) can be caused with input parameters to the Runtime function.
In some circumstances, preliminary work regarding storage is required before a specific benchmark can be reliably measured.
...
The Runtime reuses components/primitives to interact with the state storage. The behaviour and
the execution cost of those primitives can be studied (Fatemeh: studied means what?) and a Weight should be applied for each use (Fatemeh: what is a use?)
within the Runtime code.
\newline

For storage, Polkadot uses three storage types across its modules, depending on the
context:
...
\begin{itemize}
  ...
  \item \textbf{Map}: Operations on multiple values, datasets, where each entry has its
...
Which storage type to use depends on the functionality of the Runtime module (or its sub-processes, rather) (Fatemeh: What does this mean?).
In some cases only a single value is required. In others, multiple values need to be operated on.
\newline

Those lower level (Fatemeh: means which one?) types get abstracted over in each individual Runtime module using the \verb|decl_storage!|
macro. Therefore, each module specifies its own storage types that are used as input and output values.
...
The storage layout is mostly the same for every primitive type (Fatemeh: What are these types?), primarily differentiated
...
\begin{itemize}
  ...
  specific entries in storage, then a certain Weight should be applied for each (additional) iteration or item.
  The Runtime defines the maximum value for such cases. If it doesn't, it unconditionally has to (Fatemeh: ??)
  ...
  maximum value, as described in paragraph \ref{para:max-value}. (Fatemeh: Is this necessary? Not obvious?)
\end{itemize}

\label{para:max-value}  (Fatemeh: What is this label?)

What the maximum value should be really depends on the functionality that the Runtime function
...
understand which behaviours or conditions can result in expensive execution times,
...
The input parameters should be randomised (Fatemeh: how?) in order to cause various effects in behaviours
...
should be recorded and ideally visualised. The author should then decide on the most probable average
...
to make adjustments to the input parameters and assigned weights according to observed behaviour

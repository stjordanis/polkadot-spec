...
<\body>
  ...
  Unlike extrinsic mappings, block mappings are not created for each block.
  The Runtime sets the key <verbatim|:changes_trie> in storage with its value
  being a tuple of two 32-bit integers:
  ...
  <\itemize-dot>
    ...
    hierarchy. <verbatim|0> means that block mappings are not created at all.
    <verbatim|1> means only the regular <verbatim|digest_interval> block
    mappings are created. Any other level means that the block mappings are
    created every <verbatim|<math|digest_interval<rsup|level>>> block for
    each level in <verbatim|1> to <verbatim|digest_levels>.
  </itemize-dot>
  ...
  Whenever the interval and level stored in <verbatim|:changes_trie> changes
  the current interval is reset and an new digest according to the changed
...
<\references>
  <\collection>
    ...
    <associate|sect-ext-storage-changes-root|<tuple|A.1.8|?>>
  </collection>
</references>
